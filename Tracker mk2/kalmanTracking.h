...
public:
	...
	CFitsTo(CMovingObject* pFit, double distance)	{
...
public:
	...
	CvSeq* contour;					// pointer to the contour in contourStorageOld.
									// makes no sense if contourStorageOld isn't up-to-date and valid. so take
									// care how you use it
	...
	CMovingObject(unsigned long ID, CvRect rect, int TTL,
		KalmanParameters kalmanInitial, CvSeq* contour)
	{
		this->ID = ID;
		this->rect = rect;
		this->TTL = TTL;
	...
	CMovingObject(unsigned long ID, CvRect rect, int TTL, CvSeq* contour):
		kalmanX( NULL ),
			kalmanY(NULL)
	{
		this->ID = ID;
