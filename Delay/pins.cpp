...
HRESULT CPinOutput :: InitAllocator(IMemAllocator **ppAlloc)	{
	...
	m_pIPPAllocator = new CIPPAllocator_8u_AC4(m_pDelayFilter, NAME("IPPAllocatorOut"), NULL, &hr);
...
HRESULT CPinOutput::InitializeAllocatorRequirements()	{
	...
	CDelayFilter* pFilt = (CDelayFilter*) m_pFilter;
	...
	pFilt->outputAllocProps.cbBuffer = pFilt->frameSize.height*pFilt->step_8u_AC4;
...
STDMETHODIMP CPinInput::GetAllocatorRequirements(ALLOCATOR_PROPERTIES* pProps)
{
	...
	pProps->cbBuffer = pFilt->inputAllocProps.cbBuffer = pFilt->frameSize.height*pFilt->step_8u_AC4;
