...
CPinOutput::CPinOutput( CMorphologicalsFilter* pFilter, HRESULT * pHr ) :
...
CPinInput::CPinInput( CMorphologicalsFilter* pFilter, HRESULT * pHr )
...
HRESULT CPinOutput::InitializeAllocatorRequirements()	{
	...
	m_pFilter->outputAllocProps.cbBuffer = pVih->bmiHeader.biHeight*(m_pFilter->step_8u_C1);
	m_pFilter->outputAllocProps.cBuffers = 3;
...
HRESULT CPinInput::InitializeAllocatorRequirements()	{
	...
	m_pFilter->inputAllocProps.cbBuffer = m_pFilter->step_8u_C1 * pVih->bmiHeader.biHeight;
	m_pFilter->inputAllocProps.cbPrefix = 0;
	m_pFilter->inputAllocProps.cBuffers = 5;
...
HRESULT CPinOutput::Deliver(IMediaSample *pMediaSample)
{
	...
	if(m_Connected == NULL) return VFW_E_NOT_CONNECTED;
...
HRESULT CPinOutput::DeliverEndOfStream()
{
	...
	if(m_Connected == NULL) return VFW_E_NOT_CONNECTED;
	if(m_pOutputQueue == NULL) return m_Connected->EndOfStream();
    m_pOutputQueue->EOS();
    return NOERROR;
}
